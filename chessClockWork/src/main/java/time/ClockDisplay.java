...
/** An interface classes that displays the move counter and receive the pause/stop signals.
 * For classes that receive the data about a player's timing, the interface
 * {@link PlayerTimeDisplay PlayerTimeDisplay} exists.
 * @author Dragonstb
 * @since 1.0;
 */
public interface ClockDisplay {

    /** Sets the move number to be displayed. Note that negative numbers are passed to indicate that
     * a number for a non-running game is to be displayed.
     * @since 1.0;
     * @param moveNo Number to be displayed.
     */
    void setMoveNumber(int moveNo);

    /** Display paused state or not.
     * @since 1.0
     * @param pause Game is paused?
     */
    void setPausedState(boolean pause);
